...
class AsgarosForumMicrodata {
    ...
    if ($this->is_microdata_enabled()) {
			...
			if (!empty($microdata_data1)) {
			 ...
			 $post = '"mainEntityOfPage": "'.esc_url($this->asgarosforum->rewrite->get_link('topic', absint($this->asgarosforum->current_topic))).'","headline": "'.esc_html(stripslashes($this->asgarosforum->current_topic_name)).'","text": "'.esc_html(wp_strip_all_tags($page_content)).'","url": "'.esc_url($this->asgarosforum->rewrite->get_link('topic', absint($this->asgarosforum->current_topic))).'","image": "'.esc_url($image_url).'","author": {"@type": "Person","name": "'.esc_html($this->asgarosforum->get_plain_username($element->author_id)).'","url": "'.get_author_posts_url( 5, wp_get_current_user()->user_login ).'"},"datePublished": "'.esc_html(mysql2date('D, d M Y H:i:s +03:00', $element->date, false)).'","dateModified": "'.$datemodified.'"';
              }
            }
           ...
           $microdata_data = false;
            ...
            if (!empty($microdata_data)) {
			 ...
			 $page_content = wp_kses_stripslashes( $page_content );
			 $page_content = str_replace("\'","\"",$page_content);
			 $post_comment[]= ',"comment": [{"@type": "Comment","text": "'.esc_html(wp_strip_all_tags($page_content)).'","author": {"@type": "Person","name": "'.esc_html($this->asgarosforum->get_plain_username($element->author_id)).'","url": "'.get_author_posts_url( 5, wp_get_current_user()->user_login ).'"},"datePublished": "'.esc_html(mysql2date('D, d M Y H:i:s +03:00', $element->date, false)).'","dateModified": "'.$datemodified.'"';
...
//}       			
	   $out = '<script type="application/ld+json">{"@context": "https://schema.org","@type": "DiscussionForumPosting",'.$post.''.implode( '', $post_comment ).''.implode( ' ', $post_comment_end ).'}</script>';		
      echo $out.PHP_EOL;
	  break;
